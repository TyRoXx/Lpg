...
static evaluate_expression_result evaluate_expression_result_create(bool const has_value, register_id const where,
                                                                    type const type_, optional_value compile_time_value,
                                                                    bool const is_pure)
{
    evaluate_expression_result const result = {has_value, where, type_, compile_time_value, is_pure};
    return result;
}
...
static read_structure_element_result read_structure_element(function_checking_state *state,
                                                            instruction_sequence *function, structure const *const type,
    ...
    if (local.exists)
    {
        if (local.where.captured_in_current_lambda.has_value)
        {
            ...
            return evaluate_expression_result_create(
                true, capture_result, local.what, local.compile_time_value, local.is_pure);
        }
        return evaluate_expression_result_create(
...
static check_function_result check_function_result_create(checked_function function, capture *captures,
                                                          size_t capture_count)
{
...
static check_function_result const check_function_result_empty = {false, {0, NULL, {NULL, 0}, NULL, 0}, NULL, 0};
                                            ...
                                            unicode_string *const parameter_names, size_t const parameter_count)
{
    ...
    *signature = function_pointer_create(
    ...
    checked_function const result =
        checked_function_create(return_value, signature, body_out, state.register_debug_names, state.used_registers);
...
static evaluate_expression_result make_compile_time_unit(void)
{
    evaluate_expression_result const result = {
        false, 0, type_from_unit(), optional_value_create(value_from_unit()), true};
    return result;
}
                                                  ...
                                                  instruction_sequence *const function, lambda const evaluated)
{
    ...
    if (checked.captures)
    {
        ...
        return evaluate_expression_result_create(true, destination, result_type, optional_value_empty, false);
    }
    ...
    if (callee.compile_time_value.is_set && all_compile_time_arguments)
    {
        ...
        }
        {
            ...
            case value_kind_function_pointer:
            {
                ...
                {
                    optional_value const compile_time_global = state->global->members[i].compile_time_value;
                                                   ...
                                                   const expression *element)
{
    ...
    if (type_equals(boolean_value.type_, result.type_))
    {
        ...
        return evaluate_expression_result_create(true, result_register, result.type_, optional_value_empty, false);
    }
    state->on_error(
                                                     ...
                                                     const expression *element)
{
    ...
    for (size_t i = 0; i < (*element).match.number_of_cases; ++i)
    {
...

        {
            ...
            {
                state->on_error(
                    semantic_error_create(semantic_error_duplicate_match_case, expression_source_begin(*case_tree.key)),
                    state->user);
                for (size_t j = 0; j < i; ++j)
                {
                    match_instruction_case_free(cases[j]);
...
evaluate_expression_result evaluate_declare_expression(function_checking_state *state, instruction_sequence *function,
                                                       const expression *element)
{
    instruction_checkpoint const before_initialization = make_checkpoint(&state->used_registers, function);
                                                      ...
                                                      expression const element)
{
    ...
    case expression_type_access_structure:
    {
        ...
        return evaluate_expression_result_create(
            true, result, element_read.type_, element_read.compile_time_value, false);
    }
